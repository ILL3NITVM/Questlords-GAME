...
const keep=(req,res)=>{if(res&&res.ok&&res.type==='basic'){const x=res.clone();caches.open(CACHE).then(c=>c.put(req,x)).catch(()=>{})}return res};
...
if(e.request.mode==='navigate'){e.respondWith(fetch(e.request).then(r=>keep(e.request,r)).catch(()=>caches.match(e.request).then(r=>r||caches.match('/'))));return;}
