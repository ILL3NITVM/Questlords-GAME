...
function mountUI() {
  if (!document.getElementById('glimmer-style')) {
    ...
    st.textContent = `
html[data-qc-build]:not([data-qc-device="iphone13"]) .top{grid-auto-flow:column!important;grid-template-columns:minmax(96px,1fr)!important;grid-auto-columns:auto!important}
...
${(() => { const v = window.__quadcomVision?.(); return v ? `<dt>VISION</dt><dd>${v.backend} · ${v.tier}${v.lastMs != null ? ` · encode ${ms(v.lastMs)}` : ''}${v.gpuMs != null ? ` · gpu done ${ms(v.gpuMs)}` : ''} · ${v.renders} renders · texture <button type="button" data-tex>${String(v.texture || 'off').toUpperCase()}</button>${v.error ? ` <span class="bad">${esc(v.error.slice(0, 40))}</span>` : ''}</dd>` : ''; })()}
